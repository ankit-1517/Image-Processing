...
def remove_horizontal_seam(sobel_img,original_img):
	newimg = np.zeros([original_img.shape[0]-1,original_img.shape[1]], dtype = np.uint8)
...
def remove_vertical_seam(sobel_img,original_img):
	newimg = np.zeros([original_img.shape[0],original_img.shape[1]-1], dtype = np.uint8)
	M = np.zeros(sobel_img.shape)
...
#function to perform seam_carving
def seam_carving_compression(img , row_dec, col_dec):
	...
	print(newimg.shape)
	#saveImage(newimg,"L",'after_vertical_seam_removal.jpg')

	for i in range(0,row_dec):
		mag = sobel_operator_to_give_energy(newimg)
		newimg , energy = remove_horizontal_seam(mag,newimg)
	print(newimg.shape)
	#saveImage(newimg,"L",'Horizontal_then_vertical_seam_removal.jpg')
	return newimg
...
def seam_carving_compression(img , row_dec , col_dec):
	...
	print(newimg.shape)
	#saveImage(newimg,"L",'Horizontal_then_vertical_seam_removal.jpg')
	saveImage(newimg,"L","seam"+str(row_dec)+"_"+str(col_dec)+".jpg")
...
saveImage(img,"L","eagle.jpg") 
seam_carving_compression(img,29,11)
seam_carving_compression(img,40,11)
seam_carving_compression(img,40,15)
seam_carving_compression(img,20,10)
seam_carving_compression(img,20,20)
seam_carving_compression(img,50,30)

# newimg = np.zeros(img.shape , dtype = np.uint8)
# newimg , energy = optimal_seam_carving_compression_generator(img,20,20)
# saveImage(newimg,"L","optimal_seam_carving_compression"+str(20)+"_"+str(20)+".jpg")
